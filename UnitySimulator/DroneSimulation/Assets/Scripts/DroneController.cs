...
public class DroneController : MonoBehaviour
{
    ...
    {
        Debug.Log($"[Mission] Stabilizing for bomb drop... Waiting for {preDropStabilizationTime} seconds.");
        // 첫 폭탄 투하 전 안정화 시간 (이 시간 동안 ApplyForcesBasedOnState의 호버링 로직이 드론을 안정시킴)
        yield return new WaitForSeconds(preDropStabilizationTime);
        ...
        for (int i = 0; i < actualBombsToDrop; i++)
        {
            if (bombPrefab != null)
            {
                // 폭탄 투하 위치 계산: 드론의 로컬 좌표계를 사용하여 오프셋 적용
                Vector3 spawnPosWorld = transform.TransformPoint(bombSpawnOffset);
                Instantiate(bombPrefab, spawnPosWorld, Quaternion.identity); // 폭탄의 초기 회전은 기본값
                currentBombLoad--;
            ...
            UnityMainThreadDispatcher.Instance().Enqueue(() =>
            {
                try
                {
                    ...
                    if (eventName == "emergency_return_command" || eventName == "force_return_command")
                    {
                        ...
                        if (droneStationLocation != null)
                        {
                            currentTargetPosition_xz = droneStationLocation.position;
                            currentMissionState = DroneMissionState.EmergencyReturn;
                    ...
                    else if (eventName == "emergency_stop_command")
                    {
                        ...
                        currentTargetPosition_xz = new Vector3(transform.position.x, 0, transform.position.z);
                    ...
                    else if (eventName == "wildfire_alert_command")
                    {
                        if (currentMissionState == DroneMissionState.IdleAtStation)
                        {
                            ...
                            currentTargetPosition_xz = wildfireGroundPos;
